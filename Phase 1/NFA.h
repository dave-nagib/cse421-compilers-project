...
class NFA {
  ...
  public:
    ...
    NFA();
    /** DI constructor */
    NFA(unordered_set<int> states, unordered_map<int, unordered_map<char, vector<int>>> transitions, int initial_state, unordered_map<int, int> accepting_states);
    ...
    unordered_set<int> eps_closure(unordered_set<int> states) const;
    /** Returns the epsilon closure of the initial state */
    unordered_set<int> get_initial() const;
    ...
    unordered_set<int> transition(unordered_set<int> states, char symbol) const;
    /**
     ...
     */
    static NFA union_nfa(vector<NFA>& nfas, int new_start, bool common_accept = false, int new_accepting = -1, int token_id = -1);
    /**
